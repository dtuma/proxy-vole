...
public class ProxyUtilTest {
	...
	public void testParseProxySettings() {
		...
		Assertions.assertThat(psList.get(0).toString()).matches(Pattern.compile("HTTP @ http_proxy\\.unit-test\\.invalid(\\/<unresolved>)?:80"));
	}
	...
	public void testParseProxySettings2() {
		...
		List<Proxy> psList = rs.select(TestUtil.HTTP_TEST_URI);
		Assertions.assertThat(psList.get(0).toString()).matches(Pattern.compile("HTTP @ http_proxy\\.unit-test\\.invalid(\\/<unresolved>)?:8080"));
	}
	...
	public void testParseProxySettings5() {
		...
		List<Proxy> psList = rs.select(TestUtil.HTTP_TEST_URI);
		Assertions.assertThat(psList.get(0).toString()).matches(Pattern.compile("HTTP @ 192.123.123.1(\\/<unresolved>)?:8080"));
	}
	...
	public void testParseIPv6WithoutProtocol() {
		...
		List<Proxy> psList = rs.select(TestUtil.HTTP_TEST_URI);
		Assertions.assertThat(psList.get(0).toString()).matches(Pattern.compile("HTTP @ 2001:4860:0:2001::68(\\/<unresolved>)?:8080"));
	}
